...
import '../service.dart';
import 'chatpagecandidate.dart';

class CandidateChat extends StatefulWidget {


  final String chatemail;

  CandidateChat(this.chatemail);

  @override
  _CandidateChatState createState() => _CandidateChatState();
}
crudMethods fobj=new crudMethods();
String recieveremail;

class _CandidateChatState extends State<CandidateChat> {
  QuerySnapshot tokendata;

  final Firestore _firestore = Firestore.instance;
  QuerySnapshot messagedata,candidatename;
  String name;
  String token;
  bool isseen=false;
  String id,date;
  ...
  Future<void> callback() async {
    ...
    candidatename= await _firestore.collection("Personal Data").getDocuments();
    for(int i=0;i<candidatename.documents.length;i++)
      {
        if(candidatename.documents[i].documentID==fobj.user.email)
          {
            name=candidatename.documents[i].data['Name'];
            break;
          }
      }
    final postUrl = 'https://fcm.googleapis.com/fcm/send';
    final data = {
      "notification": {"body": "Message", "title": "You got a message from ${name}"},
      "priority": "high",
      "data": {
  ...
  Widget build(BuildContext context) {
    ...
    return new WillPopScope(
        ...
        child:Scaffold(
          appBar: AppBar(
            title: new Center(child: new Text("Hire Easy",textAlign: TextAlign.center,style: TextStyle(fontFamily: 'BeVietnam'),),),
  ...
  Future<bool> _onWillPop() {
    return Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => CandidateChatPage()));
  }
  void showLongToast() {
